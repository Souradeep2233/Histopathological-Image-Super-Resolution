...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        ...
        noise_level = torch.FloatTensor([self.sqrt_alphas_cumprod_prev[t+1]]).repeat(batch_size, 1).to("cuda")
        x=x.to("cuda")
        condition_x=condition_x.to("cuda")
    ...
    def super_resolution(self, x_in):
        img = torch.rand_like(x_in, device="cuda")
        ...
        print(f"Time exected :{end-start}s")
        return img

    # Compute loss to train the model
    def p_losses(self, x_in,y_target):
        y_target=None
        x_start = x_in
        ...
        sqrt_alpha = torch.FloatTensor(
            np.random.uniform(self.sqrt_alphas_cumprod_prev[t-1], self.sqrt_alphas_cumprod_prev[t], size=b)).to("cuda")
        ...
        return self.loss_func(noise, pred_noise)#+.03*self.loss_func(x_in,y_target)

    def forward(self, x, y_target):
        return self.p_losses(x,y_target)
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            low_res_image = torchvision.transforms.Resize((32,32))(low_res_image)
            
            low_res_image = self.transform(low_res_image)
            high_res_image = torchvision.transforms.Resize((32,32))(high_res_image)
...
if __name__ == "__main__":
    ...
    print(device)
    schedule_opt = {'schedule':'linear', 'n_timestep':200, 'linear_start':1e-4, 'linear_end':0.05}

    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1', 
                dataloader=None, testloader=test_dataloader, schedule_opt=schedule_opt, 
                save_path=None, load_path="SR3_16_32_tissue_exp.pt", load=True, inner_channel=96, 
                norm_groups=16, channel_mults=(1, 2, 2, 2), dropout=0.2, res_blocks=4, lr=1e-5, distributed=True)
    #sr3.train(epoch=2, verbose=1)
    
    test_imgs = next(iter(test_dataloader))
    img_test_lr,_ =test_imgs
    
    print("Model Implementation in process:")
    t=sr3.test(img_test_lr) # 128s

    plt.imshow(np.transpose(torchvision.utils.make_grid(t.detach().cpu(), 
                                                                    nrow=2, padding=1, normalize=True),(1,2,0))) # 4-6 s
...
# Time Step causing the delay ! (94% of total time consumed) : No  memory leak , or gradient or any external redundancy , it's supposingly normal GPU 
#behaviour taking time to compute with more time steps !!!





# Cheeck model loading device issue !