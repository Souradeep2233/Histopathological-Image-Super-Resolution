...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        ...
        noise_level = torch.FloatTensor([self.sqrt_alphas_cumprod_prev[t+1]]).repeat(batch_size, 1).to(device)  # x.device ->device
    ...
    def train(self, epoch, verbose):
        ...
        for i in tqdm(range(epoch)):
            ...
            if (i+1) % verbose == 11:
                ...
                test_imgs_lr,test_imgs_hr = test_imgs
                test_imgs_lr=test_imgs_lr.to(self.device)
                # test_imgs_hr=test_imgs_hr.to(self.device)
                b, c, h, w = test_imgs_lr.shape

                with torch.no_grad():
                    val_loss = self.sr3(test_imgs_lr)
                ...
                grid_img = torchvision.utils.make_grid(test_imgs_lr, nrow=2, padding=1, normalize=True).cpu().numpy()
                ...
                plt.imshow(np.transpose(torchvision.utils.make_grid(self.test(test_imgs_lr).detach().cpu(), 
...
class CustomImageDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.image_folder = "Training_data_40X_100X"
...
if __name__ == "__main__":
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1', 
                ...
                save_path="SR3_16_32_tissue_exp.pt", load_path=None, load=False, inner_channel=96, 
